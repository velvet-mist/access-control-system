...
impl ResponseReader {
            ...
            match conn.read(&mut temp_buffer) {
                ...
                Ok(n) => {
                    ...
                    if self.buffer.len() > 1024 {
                        return Err(AdapterError::PlcComm("Response buffer overflow".to_string()));
                    }
                }
                Err(e) => {
    ...
    pub fn read_bytes(&mut self, conn: &mut KeyenceConnection, count: usize) -> Result<Vec<u8>, AdapterError> {
        self.buffer.clear();
        self.buffer.resize(count, 0);
        
        let mut total_read = 0;
        while total_read < count {
            match conn.read(&mut self.buffer[total_read..]) {
                Ok(0) => {
                    // Connection closed
                    break;
                }
    ...
    pub fn read_lines(&mut self, conn: &mut KeyenceConnection) -> Result<Vec<String>, AdapterError> {
        self.buffer.clear();
    ...
    pub fn is_success(response: &str) -> bool {
        ...
        upper == "OK" || upper.starts_with("OK") || upper.starts_with("0") || upper.starts_with("20")
    }
...
mod tests {
    ...
    fn test_parse_error() {
        ...
        assert_eq!(ResponseReader::parse_error("ER25"), Some("ER25".to_string()));
        assert_eq!(ResponseReader::parse_error("ERROR: timeout"), Some("ERROR: timeout".to_string()));
